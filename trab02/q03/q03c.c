...
void matrix_multiplication(int **ma, int **mb, int **mr){
    ...
    for (j = 0; j < MATRIX_B_COLUMNS; j++) {
   		args_struct *args = (args_struct*)malloc(sizeof(args_struct));
			...
			args -> ma = ma;
			args -> mb = mb;
			if(active_threads < PROCESSORS){
				printf("creating thread\n");
				pthread_create(&(tid[k]), NULL, &calc_cell, args);
				active_threads++;
			}
			if(active_threads == PROCESSORS || k == (MATRIX_A_ROWS*MATRIX_B_COLUMNS - 1)){
				int count;
				if(k == (MATRIX_A_ROWS*MATRIX_B_COLUMNS - 1)){
					printf("joining thread n: %d\n", k);
					pthread_join(tid[k], NULL);
				}else{
					for(count = 0; count < active_threads; count++){
						int num_t = count + PROCESSORS*(((k+1)/PROCESSORS)-1);
						printf("joining thread n: %d\n", num_t);
						pthread_join(tid[num_t], NULL);
					}
					active_threads = 0;
				}
			}
